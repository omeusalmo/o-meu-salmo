...
| ~~**Hardening da API key Firebase**~~ ✅ FEITO 2026-07-09 | Key `AIzaSy...PtJQ` restrita por package `com.omeusalmo.salmos` + SHA-1 (assinatura + upload) no Cloud Console (projeto `o-meu-salmo`, não `meu-salmo`/TTS). | Verificar que Analytics/Crashlytics seguem reportando no app da loja. |
