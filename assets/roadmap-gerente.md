...
| ~~Domínio omeusalmo.com.br~~ ✅ REGISTRADO E NO AR | Live via GitHub Pages (resolve p/ 185.199.108.153; raiz + `/privacy_policy.html` = HTTP 200). assetlinks.json servido → App Links funcionando (ver [[project_assetlinks]]). | Pendência menor: link in-app da política em `ajustes_screen.dart` ainda aponta p/ `omeusalmo.github.io` — trocar p/ `omeusalmo.com.br` numa próxima build (cosmético, github.io funciona). |
