...
| **Hardening da API key Firebase** | Repo é público; `google-services.json` versionado expõe a key `AIzaSy...PtJQ` (não é segredo, mas restringir é boa prática). | Cloud Console → Credenciais → restringir key por package `com.omeusalmo.salmos` + SHA-1 (assinatura do app + upload, do Play Console → Integridade do app). **Registrar os SHA no Firebase ANTES** de restringir, senão Analytics/Crashlytics quebram no app da loja. |
