...
# Áudios não são versionados (62 MB) — sincroniza da cópia local do app V1
if [ -d ../app/assets/audios ] && [ -n "$(ls -A ../app/assets/audios 2>/dev/null)" ]; then
  mkdir -p assets/audios
  rsync -a ../app/assets/audios/ assets/audios/
  echo "Áudios sincronizados de ../app/assets/audios"
fi
