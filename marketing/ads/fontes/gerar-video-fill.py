#!/usr/bin/env python3
"""Gera frames do vídeo de lançamento com fill letra-a-letra (karaokê).
Cada frame = estado determinístico do preenchimento (progresso global p in [0,1]).
Uso: python3 gerar-video-fill.py <p_0a1>  -> escreve /tmp/fill-frame.html
Modo batch (default, N frames): escreve HTMLs em /tmp/fillframes/."""
...
<div class="ad">
  ...
  <div class="top">
    ...
    <div class="ref">Salmo 23 · a narração acompanha a leitura</div>
  </div>
  <div class="reader">
"""
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] != "batch":
        open("/tmp/fill-frame.html", "w").write(frame_html(float(sys.argv[1])))
    else:
        os.makedirs("/tmp/fillframes", exist_ok=True)
        N = 60
        for k in range(N):
            open(f"/tmp/fillframes/f{k:03d}.html", "w").write(frame_html(k / (N - 1)))
        print(f"{N} frames HTML gerados")
